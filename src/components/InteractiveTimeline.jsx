              ...
              <motion.div
                ...
                <div className={`w-full md:w-5/12 ${index % 2 === 0 ? 'md:pr-8' : 'md:pl-8'}`}>
                  <motion.div
                    ...
                    className={`glass-card glass-hover p-6 rounded-xl cursor-pointer relative overflow-hidden ${
                      activeIndex === index ? 'ring-2 ring-[rgb(var(--primary))] shadow-xl shadow-[rgb(var(--primary))]/20' : ''
                    }`}>
                    
                    {/* Type badge */}
                    <div className="absolute top-4 right-4">
                      <span className="px-3 py-1 rounded-full text-xs font-medium bg-[rgb(var(--muted))] text-[rgb(var(--muted-foreground))] border border-[rgb(var(--border))]">
                    ...
                    <div className="flex items-start mb-4 relative z-10">
                      <div
                        className="w-14 h-14 rounded-xl bg-[rgb(var(--primary))]/10 flex items-center justify-center text-[rgb(var(--primary))] text-xl mr-4 border border-[rgb(var(--primary))]/20 flex-shrink-0">
                        {exp.icon}
                      </div>
                      <div className="flex-1">
                        <h3 className="text-xl font-bold text-[rgb(var(--foreground))] mb-1 leading-tight">{exp.title}</h3>
                        <p className="text-[rgb(var(--primary))] text-sm font-semibold">{exp.company}</p>
                      </div>
                    </div>

                    <div className="flex flex-wrap gap-3 text-xs text-[rgb(var(--muted-foreground))] mb-4 relative z-10">
                      <span className="flex items-center gap-1 bg-[rgb(var(--muted))]/50 px-3 py-1.5 rounded-full">
                    ...
                    <ul className="space-y-2.5 relative z-10">
                      {exp.description.map((desc, i) => (
                        <motion.li 
                          ...
                          <span className="text-[rgb(var(--primary))] mr-2 mt-0.5 font-bold">▸</span>
                          <span>{desc}</span>
                        </motion.li>
                      ))}
                    </ul>
